        ...
        for(let i = 0; i < ingredients.length; i++){
            ...
            for(let j = 0; j < this.state[buf]; j++){
                ...
                if(buf.includes("candles")){
                    if(buf.includes("60"))
                        cake.push(<div key={cake.length} className="cakeSideCandles60" style={{ zIndex: i+1, marginBottom: "-23%"}}><img src={str_buf}/></div>)
                    if(buf.includes("80"))
                        cake.push(<div key={cake.length} className="cakeSideCandles80" style={{ zIndex: i+1, marginBottom: "-27%"}}><img src={str_buf}/></div>)
                    if(!((buf.includes("60") || buf.includes("80")))){
                        cake.push(<div key={cake.length} className="cakeSideCandles100" style={{ zIndex: i+1, marginBottom: "-22%"}}><img src={str_buf}/></div>)
                    }
                }
            }
        }
